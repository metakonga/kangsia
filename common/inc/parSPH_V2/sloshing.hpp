...
public:
	...
	sphydrodynamics* initialize()
	{
		...
		isph->setParticleShifting(true, 1, 0.01f);
		...
		fluid_top->define(VEC3F(0.0f, 0.1025f, 0.f), VEC3F(0.5f, 0.1025f, 0.f), true);

		// 		geo::line *fluid_right = new geo::line(isph, FLUID, "fluid right");
		// 		fluid_right->define(VEC3F(0.1025f, 0.2025f, 0.f), VEC3F(0.1025f, 0.0f, 0.f), true);
		...
		bottom_wall->setExpressionMovement(true);

		geo::line *left_wall = new geo::line(isph, BOUNDARY, "left wall");
		left_wall->define(VEC3F(0.0f, 0.3f, 0.f), VEC3F(0.0f, 0.0f, 0.0f), true);
		...
		right_wall->define(VEC3F(0.5f, 0.0f, 0.f), VEC3F(0.5f, 0.3f, 0.f), true);

		right_wall->setExpressionMovement(true);
