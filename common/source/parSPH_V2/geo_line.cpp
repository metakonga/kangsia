...
void line::define(VEC3D& start, VEC3D& end, bool normalStartEndLeft, bool considerHP, bool _isInner)
{
	startPoint = start;
	endPoint = end;
	normal = (end - start).rotate2((((int)normalStartEndLeft) * 2 - 1)*M_PI_2).normalize();
...
void line::build(bool onlyCountParticles)
{
	...
	VEC3D diff = endPoint - startPoint;
	...
	for (int i = 1; i < lineCnt; i++){
		VEC3D displacement = startPoint + (i * spacing) * unitDiff;
...
bool line::particleCollision(const VEC3D& position, double radius)
{
