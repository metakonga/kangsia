...
resultTable::resultTable(QWidget *parent) : QWidget(parent)
{
	...
	QStringList labels;
	labels << kor("ID") << kor("N. Effective Turn") << kor("Center Diameter(mm)") << kor("Wire Diameter(mm)") << kor("Free Length(mm)") << kor("Stiffness(kgf/mm)") << kor("Mass(kg)") << kor("Block Height(mm)") << kor("Weight(kgf)") << kor("B.H. Weight(kgf)") << kor("Max. Stree(kgf/mm^2)") << kor("B.H. Stress(kgf/mm^2)") << kor("Effective Potential(N*m)") << kor("Efficiency(E)") << kor("Transter Energy(N*m)") << kor("Dead-End Velocity(m/s)") << kor("Spring Index") << kor("Aspect Ratio");
